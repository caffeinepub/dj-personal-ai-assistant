const PLAN_TEMPLATES = [
  {
    ...
    steps: [
      ...
      {
        description: "Build a small virtual network lab (GNS3 or Packet Tracer)"
      },
      {
        description: "Review security concepts: firewalls, VPNs, and protocols"
      },
      {
        description: "Prepare for a networking certification (e.g. CompTIA Network+)"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    steps: [
      {
        description: "Set a specific fitness goal (weight, strength, endurance)"
      },
      {
  ...
  },
  {
    ...
    steps: [
      {
        description: "Choose a language to start with (Python or JavaScript recommended)"
      },
      { description: "Complete a beginner course or tutorial series" },
      {
        description: "Practice daily with small coding challenges (LeetCode, HackerRank)"
      },
      { description: "Build a simple personal project from scratch" },
      { description: "Learn version control with Git and GitHub" },
      {
        description: "Explore a framework (Django, React, etc.) relevant to your goals"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        description: "Choose a task management system (GTD, time-blocking, etc.)"
      },
...
function findMatchingTemplate(goal) {
...
function generatePlanFromGoal(goal) {
  const template = findMatchingTemplate(goal);
  const steps = template ? template.steps.map((s) => makeStep(s.description)) : [
